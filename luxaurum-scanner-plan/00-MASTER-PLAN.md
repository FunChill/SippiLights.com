...
## 2. HOW WE BEAT IT — THE 8 DIFFERENTIATORS
...
8. **Compare Mode + Shelf Shot** — the in-store decision moment: a shopper holding 2–3 shelf options and wondering which is the healthy choice. Two tiers:
   - **Tier 1 — Compare Mode:** scan 2–3 items sequentially (barcode or label photo), get one ranked verdict card — "Option B is best *for your profile*" — with per-item flag summaries and better-swap links at the bottom. Reliable, ships in Phase 5.
   - **Tier 2 — Shelf Shot:** one group photo of the shelf options; AI identifies each product, falls back to "tap to confirm" when uncertain, then runs the same ranked verdict. The magic-feeling upgrade, Phase 6+ once single-label extraction is field-proven.
   - Competitive gap: Yuka has no true compare; Think Dirty is sequential-manual; RevealIt has none; small apps (ShelfSmart, Lifesum) compare two products on *nutrition* only, not personalized ingredient safety. Nobody does personalized, cited, multi-product compare — and nobody does it from one photo.
...
- **Premium (~$6.99/mo or $49.99/yr):** unlimited scans, AI chat, household profiles, **Compare Mode / Shelf Shot**, alternatives engine, exposure insights, scan history export.
